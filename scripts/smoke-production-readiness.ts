...
for (const key of ['AUTH_SECRET', 'DATA_SECRET', 'QADA_ADMIN_CREDENTIAL_HASH_V7', 'REDIS_URL', 'UPSTASH_REDIS_REST_URL', 'UPSTASH_REDIS_REST_TOKEN', 'QADA_OPEN_TEST_MODE', 'QADA_GUEST_ACCESS', 'QADA_PUBLIC_INDEXING']) {
...
assert(server.includes("const PUBLIC_INDEXING_ENABLED = process.env.QADA_PUBLIC_INDEXING === 'true';"), 'public indexing must be explicit and fail closed');
assert(server.includes("X-Robots-Tag', 'noindex, nofollow, noarchive'"), 'non-public environments must emit noindex');
assert(server.includes("Disallow: /"), 'robots.txt must block crawling when public indexing is disabled');
