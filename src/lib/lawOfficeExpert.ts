...
export type LawOfficeRoute = { task: LawOfficeTask; stage: CourtStage; draftingRequested: boolean; allowDrafting: boolean; blocking: boolean; hasJudgmentSignals: boolean; reason: string; nextAction: string; };
...
export function analyzeLawOfficeRoute(text: string, hasAttachmentEvidence = false): LawOfficeRoute {
  ...
  if (task === 'appeal' && (stage === 'appeal' || stage === 'supreme')) return { task, stage, draftingRequested, allowDrafting:false, blocking:true, hasJudgmentSignals, reason:'المادة المعروضة تبدو صادرة من مرحلة الاستئناف/العليا؛ لا يجوز للمنصة أن تنشئ لائحة استئناف جديدة تلقائياً قبل تحديد طريق الاعتراض التالي نظاماً.', nextAction:'استخرج درجة الحكم ومنطوقه وتاريخ التبليغ، ثم تحقق من طريق الطعن التالي من المصدر الرسمي؛ إذا كان المطلوب نقضاً فحوّل المهمة إلى طعن بالنقض بعد التحقق.' };
  if (task === 'cassation' && stage === 'first-instance') return { task, stage, draftingRequested, allowDrafting:false, blocking:true, hasJudgmentSignals, reason:'المادة المعروضة تبدو حكماً ابتدائياً؛ طلب النقض في هذه المرحلة يحتاج تصحيح المسار قبل الصياغة.', nextAction:'حدد طريق الاعتراض على الحكم الابتدائي أولاً، ولا تصغ طعناً بالنقض قبل التحقق من المرحلة المختصة.' };
  if (task === 'cassation' && stage === 'supreme') return { task, stage, draftingRequested, allowDrafting:false, blocking:true, hasJudgmentSignals, reason:'المادة المعروضة تبدو صادرة من المحكمة العليا/الإدارية العليا؛ لا تنشئ طعناً جديداً من النوع نفسه على الحكم دون تحديد طريق نظامي آخر.', nextAction:'تحقق من طبيعة الحكم والطرق الاستثنائية المتاحة -إن وجدت- قبل أي صياغة.' };
  if ((task === 'appeal' || task === 'cassation' || task === 'petition') && !hasJudgmentSignals) return { task, stage, draftingRequested, allowDrafting:false, blocking:true, hasJudgmentSignals, reason:'لا يمكن صياغة طريق اعتراض مهني من دون الحكم أو بياناته الجوهرية.', nextAction:'اطلب الحكم أو نص المنطوق والأسباب ودرجة المحكمة وتاريخ التبليغ، ثم ابدأ تحليل طريق الاعتراض.' };
  if ((task === 'appeal' || task === 'cassation' || task === 'petition') && stage === 'unknown') return { task, stage, draftingRequested, allowDrafting:false, blocking:true, hasJudgmentSignals, reason:'درجة الحكم غير محسومة من البيانات الحالية، ولا يكفي وجود مرفق وحده لتحديد طريق الاعتراض.', nextAction: task === 'petition' ? 'استخرج من الحكم المحكمة ودرجته وصفة الحكم ونهائيته ومنطوقه وتاريخ التبليغ، ثم تحقق من قابلية الالتماس وسببه النظامي قبل الصياغة.' : 'حدد هل الحكم ابتدائي أم استئناف أم صادر من المحكمة العليا قبل صياغة اللائحة.' };
  return { task, stage, draftingRequested, allowDrafting:true, blocking:false, hasJudgmentSignals, reason:'المسار لا يظهر فيه تعارض إجرائي أولي يمنع بدء العمل.', nextAction:'ابدأ بتحليل ملف القضية ثم الصياغة، مع إبقاء المواعيد والمواد خلف بوابة التحقق الرسمي.' };
}
