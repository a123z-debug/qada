...
export function PdfUploadModal({ isOpen, onClose, onAddAttachments, onAnalyzeImmediately }: PdfUploadModalProps) {
  ...
  const handleProcessFiles = async (files: FileList | null) => {
    ...
    setUploadError(null);
    // Inline attachments travel inside the serverless JSON request as base64.
    // Keep the raw payload conservative so base64 + JSON overhead stays below the function limit.
    const maxTotalBytes = Math.floor(2.5 * 1024 * 1024);
    const newItems: Attachment[] = [];
    let totalBytes = selectedFiles.reduce((sum, item) => sum + item.size, 0);

    for (const file of Array.from(files)) {
      if (file.size > maxTotalBytes) {
        setUploadError(`الملف "${file.name}" أكبر من الحد الآمن للإرسال المباشر (2.5 ميجابايت).`);
        continue;
      }
      if (totalBytes + file.size > maxTotalBytes) {
        setUploadError('إجمالي المرفقات تجاوز 2.5 ميجابايت. أرسل الملفات على دفعات أصغر.');
        continue;
      }
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={e => e.target === e.currentTarget && resetAndClose()}>
    <div className="relative w-full max-w-2xl bg-neutral-900 border border-neutral-700 rounded-2xl shadow-2xl overflow-hidden text-right flex flex-col max-h-[90vh]">
      ...
      <div className="p-4 border-t border-neutral-800 flex gap-3 justify-end"><button onClick={handleConfirmAndAdd} disabled={!selectedFiles.length} className="px-4 py-2 rounded-xl bg-neutral-800 text-neutral-200 disabled:opacity-40">إرفاق بالمحادثة</button><button onClick={handleConfirmAndAnalyze} disabled={!selectedFiles.length} className="px-4 py-2 rounded-xl bg-amber-500 text-black font-bold disabled:opacity-40">إرفاق وتهيئة طلب التحليل</button></div>
    </div>
  </div>;
}
