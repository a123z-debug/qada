    ...
    >
      ...
      <div className="w-full lg:w-3/4 flex-1 flex flex-col h-full overflow-hidden relative z-10">
        ...
        <main className="flex-1 overflow-y-auto p-3 sm:p-6 lg:p-8 pb-24 sm:pb-6 lg:pb-8 custom-scrollbar">
          ...
          {session.role === 'admin' && isAdminMapOpen && (
            <AdminAgentMap />
          )}

          {!activeCourt && !isAdminMapOpen && (
            <WelcomeScreen
          ...
          {!isAdminMapOpen && activeCourt === 'administrative' && (
          ...
          {!isAdminMapOpen && activeCourt === 'general' && (
            <GeneralWorkspace
              service={activeService}
              userSession={session}
            />
          ...
          {!isAdminMapOpen && activeCourt === 'criminal' && (
            <CriminalWorkspace
              service={activeService}
              userSession={session}
            />
      ...
      >
        <div className="grid grid-cols-5 gap-1 px-2 pt-2">
          <button
            type="button"
            onClick={() => { setIsAdminMapOpen(false); setActiveCourt(null); setActiveService(null); }}
