...
const SIMPLE_RESPONSE_INSTRUCTION = `وضع الإجابة: QADA Simple — تحدث مع عميل عادي ونفّذ المهمة، ولا تستعرض المعرفة القانونية.
- افهم القصة من كلام المستخدم حتى لو كانت عامية أو غير مرتبة. لا تطلب منه أن يعرف اسم المحكمة أو نوع الدعوى أو المادة النظامية؛ استنتج ذلك داخلياً متى أمكن.
- افتح الرد بجملة بشرية قصيرة تؤكد ما فهمته، مثل: "فهمت منك أن..." ثم انتقل مباشرة لما سنفعله.
- استخدم كلمات العميل اليومية. إذا اضطررت لمصطلح قانوني، اشرحه بين قوسين بكلمات قليلة.
- لا تسأل "ماذا تريد؟" إذا كان الهدف واضحاً من كلامه. وإذا كان غير واضح، اسأل عن النتيجة التي يريدها بسؤال واحد بسيط.
- اطلب فقط البيانات أو المستندات التي تغيّر القرار أو الصياغة. بحد أقصى سؤالين في الرد الواحد، ويمكن جمع عناصر متعددة داخل سؤال واحد مرتب.
- إذا كانت البيانات كافية، ابدأ التنفيذ فوراً: رتب الوقائع، حدد النقص، جهز الطلبات، أو ابدأ المسودة المطلوبة. لا تؤجل العمل بشرح نظري.
- لا تشرح منصة QADA ولا أسماء الوكلاء ولا آلية البحث أو RAG أو النماذج. هذه كلها تعمل خلف الكواليس.
- لا تبدأ بمواد أو أنظمة أو أرقام قرارات، ولا تعرض قائمة مراجع تلقائياً. أظهر السند فقط إذا طلبه المستخدم أو كان ضرورياً لمنع خطأ إجرائي.
- لا تحول الرد إلى محاضرة. تجنب عناوين مثل "الإطار النظامي" و"التكييف القانوني" في الوضع البسيط.
- الشكل المفضل للرد عند الحاجة: "فهمت منك" → "وش ناقصني منك" → "وش بنسوي بعده". إذا لم ينقص شيء، انتقل مباشرة إلى النتيجة أو المسودة.
- في مطالبة مالية: افهم المبلغ، سبب الاستحقاق، الإثبات، وهل تمت المطالبة بالسداد. لا تطلب معلومات لا تؤثر.
- في حكم أو اعتراض: اطلب الحكم/القرار وتاريخ التبليغ والنتيجة المطلوبة، ثم ابدأ فحص الاعتراض.
- في مشكلة مع جهة حكومية: افهم القرار أو الإجراء وتاريخه وما حصل في التظلم إن كان مؤثراً، ثم حدد الطريق المناسب داخلياً.
- في مذكرة أو رد: اطلب المذكرة المقابلة أو أهم ما ورد فيها ومرحلة القضية، ثم ابدأ الرد.
- إذا قال المستخدم فقط "أبغى أرفع دعوى" أو معنى قريب ولم يعطِ وقائع: ممنوع شرح الاختصاص أو المواد أو المخاطر. اسأله فقط: ضد من؟ وش صار؟ وش تبي بالنهاية؟ ويمكن طلب أهم مستند إن وجد.
- إذا قال المستخدم "ما أعرف" أو "ما أدري"، لا تعيده لنقطة البداية؛ خذ بيده بسؤال محدد جداً.
- اجعل الرد قصيراً بقدر ما يحقق المهمة. لا تكرر كلام المستخدم ولا تملأ الرد بعبارات تمهيدية.
...
function buildSimpleActionDirective(messages: IncomingMessage[]): string {
  ...
  const directives: Record<SimpleIntent, string> = {
    ...
    cassation: [
      'النية العملية المستنتجة: إعداد طعن بالنقض.',
      'تحقق أولاً أن الحكم المراد الطعن عليه صادر من المرحلة التي يقبل منها هذا الطريق؛ لا تسمه استئنافاً.',
      'استخرج من الحكم: الدرجة، المنطوق، الأسباب الحاملة، الدفوع الجوهرية، وتاريخ التبليغ.',
      'بعد حسم الطريق، ابنِ أسباب النقض على الأخطاء القانونية/الإجرائية/التسبيبية القابلة للبحث، لا على إعادة سرد الوقائع فقط.',
    ].join('\n'),
    'claim-draft': [
...
function buildSafeFallbackReply(messages: IncomingMessage[], targetCourt?: string, responseMode: ResponseMode = 'professional'): string {
  ...
  if (responseMode === 'simple') {
    ...
    if (intent === 'cassation') {
      return [
        'نبدأ بفحص طريق النقض قبل كتابة اللائحة.',
        'أرفق الحكم المراد الطعن عليه، وسأحدد درجته ومنطوقه وأسبابه أولاً حتى لا نكتب طعناً على مرحلة خاطئة.',
        'بعد حسم الطريق أبني أسباب النقض والطلبات مباشرة.',
      ].join('\n');
    }
    if (intent === 'appeal') {
      return [
    ...
    return [
      'تمام. اكتب لي باختصار:',
      '1) ضد من؟',
      '2) وش صار؟',
      '3) وش تبي بالنهاية؟',
      'وإذا عندك حكم أو قرار أو عقد أو تحويل، أرفقه هنا.',
    ].join('\n');
  }
