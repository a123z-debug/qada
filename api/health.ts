...
function hasAiProvider() {
  return Boolean(
    (process.env.AI_GATEWAY_API_KEY || process.env.VERCEL_OIDC_TOKEN || '').trim()
      || [1, 2, 3, 4].some((index) => Boolean(process.env[`GEMINI_API_KEY${index === 1 ? '' : `_${index}`}`]?.trim()))
  );
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const ready = authConfigured && adminConfigured && aiConfigured && redisConfigured && redisReachable;
